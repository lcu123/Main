...
- Not yet run: `reserve_slot`→`schedule_appointment`→`cancel_appointment`→`complete_appointment` (the first two blocked on the scheduling-permission gap above; cancel/complete are irreversible/side-effectful and need an explicit go-ahead on a disposable appointment).
