...
- Responses are JSON `{success, errorMessage, ...}`; HTTP is 200 even on failure. `search` returns `idName`/`propertyName` plus the ID list under `{entity}IDs`, and with `includeData=1`, the first 1000 resolved records under the entity's own key; the rest come via `get` in chunks of 1000.
...
pip install -e ".[dev]"
pytest                                  # fake-API wire-format tests, must stay green (54 tests)
