...
- **Worth revisiting**: `due_for_service`'s default `limit=200` produced a 200K+ character response against Zest's real subscription volume -- too large for a single tool response in practice. Lower the default, or trim `SUBSCRIPTION_KEYS` further for this tool specifically. Not yet fixed.
